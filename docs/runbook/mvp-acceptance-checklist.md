...
- [x] 微信开发者工具已安装（Stable 2.01.2510290+）
- [x] 真机已安装微信，已添加为小程序体验者
- [x] 真机 openid 已在 `OPENID_ALLOWLIST` 中（参考 `docs/runbook/cloud-setup.md` §4.2）
- [x] 真机可正常打开小程序首页
...
>
> 注意：Part 1 主要验证录音 → OSS → Worker → 本地落盘的数据链路完整性。试听 / 暂停 / 重录 / 删除 / 切后台这类 US-009 真机交互生命周期，需要继续执行 Part 5.4；Part 1 通过不代表试听生命周期 AC 已通过。
...
### 5.4 试听生命周期跨端 smoke test

> **目标**：验证 iOS 与 Android 使用同一套小程序实现时，草稿试听资源在重复试听、暂停、重录、删除、保存并上传、切后台路径上都能释放干净，不产生 `operateAudio` error 级别控制台噪声。

记录测试环境：

| 平台 | 设备 / 系统版本 | 微信版本 | 基础库版本 | 结果 |
|---|---|---|---|---|
| iOS | | | | |
| Android | | | | |

如暂时没有 Android 设备，必须在结果栏明确写 `未执行（无设备）`，不能默认视为通过。

#### iOS

1. iOS 真机打开小程序首页，连接 DevTools / vConsole 日志
2. 打开 iPhone 实体静音开关 / 静音模式
3. 录制 5 秒 → 停止，进入草稿确认态
4. 连续点击「试听」2 次 → 点击「暂停」
5. 点击「重录」→ 再录制 5 秒 → 停止
6. 点击「试听」→ 点击「暂停」→ 点击「删除」并确认
7. 再录制 5 秒 → 停止 → 点击「试听」→ 点击「保存并上传」
8. 将小程序切到后台，再切回前台
   - [ ] 全流程可继续操作，无页面卡死
   - [ ] 静音模式下每次「试听」都能实际听到草稿录音
   - [ ] 控制台无 `operateAudio:fail jsapi has no permission` error 级别日志
   - [ ] 删除路径没有上传记录残留
   - [ ] 保存并上传路径最终进入上传列表并继续走上传 / verify 流程

#### Android

1. Android 真机打开小程序首页，连接 DevTools / vConsole 日志
2. 执行与 iOS 完全相同的 5 秒录音 → 试听 → 暂停 → 重录 → 删除 → 保存并上传 → 切后台 / 前台流程
   - [ ] 全流程可继续操作，无页面卡死
   - [ ] 控制台无 `operateAudio` 相关 error 级别日志
   - [ ] 删除路径没有上传记录残留
   - [ ] 保存并上传路径最终进入上传列表并继续走上传 / verify 流程
...
| iOS / Android 试听生命周期 smoke test | 无 `operateAudio` error，删除 / 重录 / 保存路径状态正确 | |
