...
`git status --porcelain` 检查结论:输出仅含 `.planning/` 路径条目,无任何仓库根文件或代码目录变更(Pitfall 6 附加核查通过)。

### 机械对账(每条给出可复算命令 + 数字)

1. **矩阵总行数** — `grep -cE '^\| [0-9]+\. ' .planning/audit/CONTRACT-MATRIX.md` → **51**(组① 15 + 组② 28 + 组③ 5 + 普查 3 = 51 ✓,与 02-02 完成判定一致)
2. **证据引用总数** — `grep -o '@ 5927f36' .planning/audit/CONTRACT-MATRIX.md | wc -l` → **236**(其中 1 处为本条命令原文自指,正文证据引用 **235**;02-04 判定列不新增行号证据——判定引用 F-CON,证据行号住台账,per D-09)
3. **diverge/absent 格数 ↔ F-CON 对账** — diverge/absent 判定格数 = **12**(组① 行 2/4/5/13 各 1 格 + 组② 行 35-41 共 7 格 + 组③ 行 46 共 1 格);`grep -c '^### F-CON-' .planning/audit/findings/contract.md` → **7**,扣除 F-CON-00 schema 示例 = **6 条**真实发现;矩阵 F-CON 链接格数 `grep -oE '→ F-CON-0[1-9]' .planning/audit/CONTRACT-MATRIX.md | wc -l` → **12**。对账等式:**12 格 = 5 条 × 1 格(F-CON-01/02/03/04/06)+ 1 条 × 7 格(F-CON-05 覆盖行 35-41 同根因)= 12 ✓**;负面清单排除的 diverge/absent 格数 = **0**(DNF/文档化约定对照点行 14/20/21 及 size=0 行 18 均为 agree 格,排除注在判定列,不占 diverge/absent 计数)
4. **判定列回填完备性** — 表格行内残留待判定数 `grep -cE '^\| [0-9]+\..*\| 待判定 \|$' .planning/audit/CONTRACT-MATRIX.md` → **0**(全文行级计数 → 2,均非表格行:判定列规则句的历史描述 1 行 + 本条命令原文自指 1 行)✓
5. **样本表对账** — 附录样本行数 `grep -cE '^\| S-[0-9]+ \|' .planning/audit/CONTRACT-MATRIX.md` → **18**;销号数 `grep -cE '^\| S-[0-9]+ \|.*✅ \|$' .planning/audit/CONTRACT-MATRIX.md` → **18**;18 = 18 ✓(≥ D-07 全 11 类下限;销号计数用行首 S- 锚定,免疫本节命令原文自指)
6. **普查对账** — 候选清单 9 项 vs 结论数:新行 1(候选 4)+ 指针 4(候选 1/5/6/9)+ 已检查无新发现 4(候选 2/3/7/8)= 9 ✓(02-02 完成判定沿用,02-04 无增删)
7. **四类分布合计** — 潜伏 2 + 覆盖洞 3 + 良性 1 = 6 条 F-CON;活跃失配 0 条(02-03 已实证当前上传链 AC#4 约束下错位 key 不进 OSS,主链无行为分叉)✓

### 成功判据自查清单(ROADMAP Phase 2 五条)

| # | 成功判据 | 状态 | 对应章节/文件 |
|---|----------|------|----------------|
| 1 | 漂移矩阵完成:每要素 × 三列,每格 agree/diverge/absent + 行号证据 | ✓ | 本文件组①/②/③ + 普查新行(51 行,agree 格同样带行号 per D-11) |
| 2 | 往返校验结论在案:FC 签发 key 能否被 Worker 解析有明确记录 | ✓ | 本文件『往返校验结论』总结论(15 python 样本同收同拒、往返等式全部成立) |
| 3 | 每条分歧归入四类之一 + Postel 宽严分析 | ✓ | 判定列(12 格四类标签 + F-CON 链接)+ `findings/contract.md` F-CON-01~06 证据字段(Postel 三要素:谁严谁宽/失配方向/触发条件) |
| 4 | 三处之外契约相关跨语言重复实现普查完成,含"已检查,无新发现" | ✓ | 本文件『重复逻辑普查』(5 命令存档 + 9 候选三态 + D14-1~6 移交) |
| 5 | 有真实分歧则黄金样本契约测试设计配方成文;无则显式"无需配方" | ✓ | 触发线裁决(本章节首)+ `.planning/audit/CONTRACT-TEST-RECIPE.md`(D-16 五要素) |

---
*契约漂移矩阵: 2026-07-05(51 行落格,判定列全回填:12 个 diverge/absent 格 → 6 条 F-CON(良性 1/潜伏 2/活跃失配 0/覆盖洞 3);18 样本全销号;配方已产出 CONTRACT-TEST-RECIPE.md;零 diff 验证通过,机械对账 7 条等式全平——Phase 2 产物封版)*
