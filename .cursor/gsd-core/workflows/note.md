...
- **Global scope**: `/Volumes/Data/ProjectCode/my_soniscope/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Volumes/Data/ProjectCode/my_soniscope/.cursor/notes/`)
...
2. Glob `/Volumes/Data/ProjectCode/my_soniscope/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (/Volumes/Data/ProjectCode/my_soniscope/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `/Volumes/Data/ProjectCode/my_soniscope/.cursor/notes/` — works in any directory
